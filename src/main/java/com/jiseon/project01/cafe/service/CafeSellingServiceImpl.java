...
import com.jiseon.project01.cafe.dao.CafeSellingDao;
import com.jiseon.project01.cafe.dto.CafeSellingDto;

@Service
public class CafeSellingServiceImpl implements CafeSellingService {
	
	@Autowired
	private CafeSellingDao sellingDao;
	...
	public void saveContent(CafeSellingDto dto, HttpServletRequest request) {
		...
		//CafeSellingDao를 이용해서 DB에 저장하기
		sellingDao.insert(dto);
	...
	public void getList(HttpServletRequest request) {
		...
		// CafeSellingDto 객체에 startRowNum, endRowNum을 담는다.
				CafeSellingDto dto = new CafeSellingDto();
		...
		if(! keyword.equals("")) {
			...
			if(condition.equals("title_content")) { // 제목+내용 검색인 경우
				//검색 키워드를 CafeSellingDto에 담아서 전달
		...
		List<CafeSellingDto> list = sellingDao.getList(dto);
		
		//전체 글의 갯수(검색 키워드가 있는 경우 키워드에 부합하는 전체 글의 갯수)
		int totalRow = sellingDao.getCount(dto);
	...
	public void getDetail(HttpServletRequest request) {
		...
		//글 하나의 정보를 얻어온다
		CafeSellingDto resultDto = sellingDao.getDataWKey(dto);
		//특수 기호를 인코딩한 키워드를 미리 준비한다.
		String encodedK = URLEncoder.encode(keyword);
	...
	public void deleteContent(int num, HttpServletRequest request) {
		...
		String id = (String)request.getSession().getAttribute("id");
		//주어진 num이용해서 정보 얻어오기
		CafeSellingDto dto = sellingDao.getData(num);
