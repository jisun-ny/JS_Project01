...
import com.jiseon.project01.cafe.dao.CafeWantedDao;
import com.jiseon.project01.cafe.dto.CafeWantedDto;

@Service
public class CafeWantedServiceImpl implements CafeWantedService {
	
	@Autowired
	private CafeWantedDao wantedDao;
	...
	public void saveContent(CafeWantedDto dto, HttpServletRequest request) {
		...
		//CafeWantedDao를 이용해서 DB에 저장하기
		wantedDao.insert(dto);
	...
	public void getList(HttpServletRequest request) {
		...
		// CafeWantedDto 객체에 startRowNum, endRowNum을 담는다.
				CafeWantedDto dto = new CafeWantedDto();
		...
		List<CafeWantedDto> list = wantedDao.getList(dto);
		
		//전체 글의 갯수(검색 키워드가 있는 경우 키워드에 부합하는 전체 글의 갯수)
		int totalRow = wantedDao.getCount(dto);
	...
	public void getDetail(HttpServletRequest request) {
		...
		//글 하나의 정보를 얻어온다
		CafeWantedDto resultDto = wantedDao.getDataWKey(dto);
		//특수 기호를 인코딩한 키워드를 미리 준비한다.
		String encodedK = URLEncoder.encode(keyword);
	...
	public void deleteContent(int num, HttpServletRequest request) {
		...
		String id = (String)request.getSession().getAttribute("id");
		//주어진 num이용해서 정보 얻어오기
		CafeWantedDto dto = wantedDao.getData(num);
