package com.jiseon.project01.users.service;
...
import com.jiseon.project01.users.dao.UsersDao;
...
public class UsersServiceImpl implements UsersService{
	...
	public void loginProcess(UsersDto dto, HttpSession session) {
		...
		//아이디를 이용해서 회원 정보를 얻어옴
		UsersDto resultDto = dao.getData(dto.getId());
		//만일 select된 회원 정보가 존재하고
		if(resultDto != null) {
			//Bcryt 클래스의 static 메소드를 이용해서 입력한 비밀번호와 암호화해서 저장된 비밀번호의 일치여부를 알아냄
	...
	public void updateUserPwd(HttpSession session, UsersDto dto, ModelAndView mView) {
		...
		if(isValid) {
			...
			//dto에 로그인 돈 아이디도 넣어준다.
			dto.setId(id);
			//dao를 이용하여 DB에 수정 반영한다.
			dao.updatePwd(dto);
			//로그아웃 처리
			session.removeAttribute("id");
		}
		//작업의 성공 여부를 ModelAndView 객체에 담아 놓는다
